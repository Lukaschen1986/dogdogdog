...
def process_imgs_with_batch(names, train_dir):
	count = 0
	with open('files/new_train.txt', 'w') as fw:
		for idx in range(len(names)):
			img = DataAugmentation.openImage(os.path.join('data/imgs/' + train_dir,names[idx]))
			new_img = DataAugmentation.resizeImage(img, 256)
			img = DataAugmentation.resizeImage(img, 224)
			
			new_name = 'data/imgs/new_' + train_dir + '/{0:0>7}.jpg'.format(count)
...
if __name__ == '__main__':
	if DEBUG:
		...
		count = 0
		process_imgs_with_batch(names, 'subject_train')
		print 'Have process subject train files.'
		process_imgs_with_batch(names, 'head_train')
		print 'Have process head train files.'
