...
#导入测试集图像
def load_test_data(transformer, data_type):
    ...
    for row in rows:
        row = row.strip('\n').split(' ')
        #读入测试集图像
        img = caffe.io.load_image(os.path.join('data/imgs/', data_type, row[0]))
        #对每张图像先进行预处理
...
if __name__ == '__main__':
    ...
    for idx in range(folds):
        ...
        #网络的权值文件
        model_weights = 'models/subject/Baidu_' + net_model + '_' + str(idx) + '_iter_10000.caffemodel'
        #导入网络模型
        ...
        #在第一个模型的时候导入测试数据
        if idx == 0:
            x_test, y_test, names = load_test_data(transformer, 'subject_train')
    ...
    for idx in range(4):
        ...
        model_deploy = 'prototxt/' + net_model + '_deploy.prototxt'
        model_weights = 'models/context/Baidu_' + net_model + '_' + str(idx) + '_iter_30000.caffemodel'
        net = caffe.Net(model_deploy, model_weights, caffe.TEST)
        transformer = caffe.io.Transformer({'data': net.blobs['data'].data.shape})
        transformer.set_transpose('data', (2, 0, 1))
        transformer.set_mean('data', mu)
        ...
        if idx == 0:
            x_test, y_test, names = load_test_data(transformer, 'context_train')
            x_test = np.array(x_test)
        pred_prob = np.zeros((x_test.shape[0], class_num))
        k = 0
        while k + batch_size < x_test.shape[0]:
    ...
    for idx in range(0, 5):
        ...
        model_deploy = 'prototxt/' + net_model + '_deploy.prototxt'
        model_weights = 'models/context/Baidu_' + net_model + '_' + str(idx) + '_iter_10000.caffemodel'
        net = caffe.Net(model_deploy, model_weights, caffe.TEST)
        transformer = caffe.io.Transformer({'data': net.blobs['data'].data.shape})
        transformer.set_transpose('data', (2, 0, 1))
        transformer.set_mean('data', mu)
    ...
    for idx in range(5):
        ...
        model_deploy = 'prototxt/' + net_model + '_deploy.prototxt'
        model_weights = 'models/head/Baidu_' + net_model + '_' + str(idx) + '_iter_10000.caffemodel'
        net = caffe.Net(model_deploy, model_weights, caffe.TEST)
        transformer = caffe.io.Transformer({'data': net.blobs['data'].data.shape})
        transformer.set_transpose('data', (2, 0, 1))
        transformer.set_mean('data', mu)
        ...
        if idx == 0:
            x_test, y_test, names = load_test_data(transformer, 'head_train')
            x_test = np.array(x_test)
        pred_prob = np.zeros((x_test.shape[0], class_num))
        k = 0
        while k + batch_size < x_test.shape[0]:
...
#######ResNet101模型
    ...
    for idx in range(5):
        ...
        model_deploy = 'prototxt/' + net_model + '_deploy.prototxt'
        model_weights = 'models/Baidu_' + net_model + '_' + str(idx) + '_iter_5000.caffemodel'
        net = caffe.Net(model_deploy, model_weights, caffe.TEST)
        transformer = caffe.io.Transformer({'data': net.blobs['data'].data.shape})
        transformer.set_transpose('data', (2, 0, 1))
        transformer.set_mean('data', mu)
